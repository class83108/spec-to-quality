...
align 完成後，報告存於 `docs/alignment/`。`feature-planning` 逐一處理報告中的 findings，每個 finding 完成後更新報告狀態，必要時更新 SYSTEM_MAP，全部 findings 完成後才將報告 archive 至 `docs/alignment/archive/`。**SYSTEM_MAP 是唯一的 ground truth**；需要重新稽核時，再跑 align 系列即可。
...
對齊完成後，`feature-planning` 是 alignment findings 的分流站——不是「決定做什麼 feature」，而是「決定怎麼處理每個 gap」：

每個 finding 根據大小走不同路徑：

| Finding 性質 | 路徑 | 工作流 |
|---|---|---|
| 缺少 contract / endpoint / 新行為 | **OpenSpec 路徑** | 開 worktree → feature plan → feature-coverage → gherkin → TDD |
| 命名 Drift / 小文件不一致 / 單一驗證缺失 | **Fix 路徑** | 開 fix worktree → 直接修 → verify → 刪 worktree |

每個 finding 完成後，更新 alignment report 的「處理進度」區塊，必要時更新 SYSTEM_MAP。全部 findings 都處理完後，報告自動 archive。SYSTEM_MAP 始終是唯一的 ground truth。
