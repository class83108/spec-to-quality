...
description: >
  Guide the creation of design-driver-discovery.md — identify which flows and pressures truly drive
  architecture decisions. Starting from goals.md, frame key flows, discover where pressure
  concentrates, select the few design drivers that matter most, and capture the implications that
  should shape system boundaries and later specs.
...
- `design-driver-discovery.md` answers: 哪些 flow 與壓力會主導設計
...
- [ ] `design-driver-discovery.md` with the sections: System Pressure Overview, Flow Framing, Pressure-Bearing Flows, Design Drivers, Design Implications, Anti-Patterns, Theory Limits, Open Questions
...
Start by framing the important work the system needs to support. Do not begin with implementation details.
...
A first-pass flow map with 5-10 coarse flows is enough. Do not force detailed step-by-step sequence diagrams.
...
Do not jump to full architecture yet. Stay at the level of design pressure and boundary implications.
...
### Phase 5: Anti-Patterns And Theory Limits
...
3. **Architecture usefulness** — Would a person writing `SYSTEM_MAP.md` know what boundaries and decisions deserve attention from this document?
4. **False importance check** — Did you accidentally elevate a symptom instead of a real design driver?
5. **Missing operational pressure check** — Did you include only user-facing flows and ignore background or coordination-heavy work?
